...
class FrontendController extends Controller
{
    ...
    public function rgpdIndex()
    {
        $page_title = 'RGPD Post';
        $empty_message = 'No rgdp post yet.';

        $titles = Frontend::where('key', 'rgpd.title')->first();

        $rgpd_posts = Frontend::where('key', 'rgpd.post')->latest()->paginate(config('constants.table.default'));
        return view('admin.frontend.rgpd.index', compact('page_title', 'empty_message', 'rgpd_posts', 'titles'));
    }

    public function rgpdNew()
    {
        $page_title = 'New Rgpd';
        return view('admin.frontend.rgpd.new', compact('page_title'));
    }

    public function rgpdEdit($id)
    {
        $page_title = 'Edit Rgpd';
        $post = Frontend::findOrFail($id);
        return view('admin.frontend.rgpd.edit', compact('page_title', 'post'));
    }
    ...
    public function translate(){
        $page_title = 'Traduction Frontend Manager';
        $langs = Language::all();
        $fronts = Frontend::all();
        return view('admin.frontend.translate', compact('page_title', 'langs', 'fronts'));
    }

    public function translateLoadFrontend($key, $lang){
        $front = Frontend::where('id', $key)->first();
        
        return view('admin.frontend.translate-frontend', compact('lang', 'front'));
    }
    ...
    public function translateUpdate(Request $request){
       ...
       $front = Frontend::where('id', $data['frontend'])->first();

       if($front){

            foreach($data as $key => $value){
                if($key !== 'lang' && $key !== 'frontend' && $key !== '_token'){
                    $req[$key] = $value;
                }
            }

            $frontValue = (array)$front->value;
            if(!isset($frontValue['lang'])){
                $frontValue['lang'] = [];
                $frontValue['lang'][$request->request->get('lang')] = $req;
            }else{
                $frontValue['lang']->$lang = $req;
            }
            ...
            //dd('dddd');
            $front->save();
            $notify[] = ['success', 'Save successly.'];
       }else{
        $notify[] = ['danger', 'Frontend don\'t exist.'];
       }
